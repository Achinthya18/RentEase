...
def signup():
    if request.method == "POST":
        ...
        if len(email) < 4:
            ...
            flash(
                "Password must contain only alphanumeric characters.", category="error"
            )
...
def home():
    ...
    if lid:
        # Check if there's any rent due in the future (PayDate > current date) and is unpaid
        cur.execute('''SELECT Tenant.TFname, Tenant.TLname, Rent.Rent, Rent.PaymentStatus, Rent.PayDate, Tenant.Tid, Rent.Lid
FROM Rent
JOIN Tenant ON Rent.Tid = Tenant.Tid
WHERE Rent.Lid = %s AND Rent.PayDate >= CURDATE() AND Rent.PaymentStatus = 'Unpaid';
 ''', (lid,))
        rent_due_soon = cur.fetchall()
        ...
        if properties:
            print(properties[0])
            if rent_due_soon:
                # If there's rent due soon, render a different page
                return render_template('rentDueSoon.html', tenant_data=rent_due_soon,properties=properties)
            # Pass the fetched properties to the template for rendering
            return render_template('home.html', properties=properties)
        
    
    # If no landlord ID is found in the session, or no properties exist, render the default page
...
def login():
    if request.method == "POST":
        ...
        try:
            ...
            if data:
                lid, stored_password = (
                    data  # Fetch Lid and stored password from database
                )
...
def tenantpage(pid):
    ...
    cur.execute(
        """SELECT TFname, TLname, Rent, PaymentStatus, PayDate , Tenant.Tid , Lid
                    FROM Tenant
    ...
    return render_template(
        "tenantpage.html", tenant_data=tenant_data, property_details=property_details
    )
...
def tenantform():
    if request.method == "POST":
        ...
        try:
            ...
            return redirect(
                url_for("auth.tenantpage", pid=session["pid"])
            )  # Redirect to home page after successful insertion
        except Exception as e:
            conn.rollback()
...
def profile():
    lid = session["lid"]
    cur.execute(
        """ SELECT LFname,LLname,LEmail,LPhone,LAddress
                  FROM Landlord
                        WHERE Lid = %s""",
        (lid,),
    )
